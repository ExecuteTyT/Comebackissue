#!/bin/bash
# Скрипт для обновления кода на сервере
# Использование: chmod +x update-server.sh && ./update-server.sh

set -e

echo "🚀 Обновление вернистраховку.рф на сервере..."

# Цвета для вывода
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m' # No Color

# Проверяем, что мы в правильной директории
if [ ! -d "/var/www/verni-strahovku" ]; then
    echo -e "${RED}❌ Директория /var/www/verni-strahovku не найдена!${NC}"
    echo "Убедитесь, что вы находитесь на сервере и проект развернут."
    exit 1
fi

cd /var/www/verni-strahovku

# Шаг 1: Получаем последние изменения из репозитория
echo -e "${YELLOW}📥 Получение изменений из GitHub...${NC}"
git fetch origin
git pull origin main

# Шаг 2: Устанавливаем зависимости (если package.json изменился)
echo -e "${YELLOW}📦 Проверка зависимостей...${NC}"
npm install --production

# Шаг 3: Перезапускаем приложение через PM2
echo -e "${YELLOW}🔄 Перезапуск приложения...${NC}"
pm2 restart verni-strahovku

# Шаг 4: Проверяем статус
echo -e "${YELLOW}✅ Проверка статуса...${NC}"
sleep 2
pm2 status

echo ""
echo -e "${GREEN}✅ Обновление завершено!${NC}"
echo ""
echo "Проверьте логи:"
echo "  pm2 logs verni-strahovku --lines 50"
echo ""
echo "Проверьте статус:"
echo "  pm2 status"
echo "  curl http://localhost:3000"


